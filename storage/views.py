...
class ResourceView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        resource_meta = storage_service.get_resource_meta(
            path=path, user_id=cast(int, request.user.id)
        )
    ...
    def delete(self, request: Request) -> Response:
        storage_service = StorageService()
        storage_service.delete_resource(
            request.query_params["path"], user_id=cast(int, request.user.id)
        )
...
class ResourceDownloadView(APIView):
    ...
    def get(self, request: Request) -> FileResponse:
        storage_service = StorageService()
        resource = storage_service.download_resource(
            request.query_params["path"], user_id=cast(int, request.user.id)
        )
        return FileResponse(
            resource, content_type="application/octet-stream", status=status.HTTP_200_OK
        )
...
class ResourceSearchView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        resources_meta = storage_service.search_resources(
            request.query_params["query"], user_id=cast(int, request.user.id)
        )
...
class DirectoryView(APIView):
    ...
    def get(self, request: Request) -> Response:
        storage_service = StorageService()
        path = request.query_params["path"]
        directory_content = storage_service.get_directory_content(
            path=path, user_id=cast(int, request.user.id)
        )
    ...
    def post(self, request: Request) -> Response:
        ...
        storage_service = StorageService()
        created_dir_meta = storage_service.create_directory(
            path=path, user_id=cast(int, request.user.id)
        )
